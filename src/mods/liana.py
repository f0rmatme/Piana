...
def extract_assets(settings: Settings):
    ...
    if settings.assets_path.joinpath("exported.yo").exists():
        ...
        # Export Models
        subprocess.call([settings.umodel,
                        f"-path={settings.game_path}",
                         f"-game=valorant",
                         f"-aes={settings.aes}",
                         "-export",
                         "*.uasset",
...
def extract_data(settings: Settings, export_directory: str, asset_list_txt: str = ""):
    ...
    args = [settings.cue4extractor.__str__(),
            ...
            "--game-umaps", settings.umap_list.__str__()
            ]
...
def import_umap(settings: Settings, umap_data: dict, umap_name: str):
    ...
    if not settings.debug:
        with redirect_stdout(stdout):
            ...
            logger.info(f"Saved : {umap_name}.blend to {shorten_path(map_path, 4)}")

            if settings.local_textures:
                bpy.ops.file.pack_all()
...
def import_map(import_settings):
    """
    Main function
    Args:
    ...
    for umap_json_path in umap_json_paths:
        ...
        remove_duplicate_mats(logger=logger)
    ...
    if settings.debug:
        ...
        post_setup(settings, SELECTED_MAP=settings.selected_map, map_list=umap_json_paths, combine=settings.combine)
...
def combine_umaps(settings: Settings):
    ...
    for umap in settings.umap_list:
